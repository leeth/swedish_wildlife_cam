...
class InfrastructureManager:
    ...
    def setup(self) -> bool:
        ...
        try:
            ...
            if not self._setup_cloudformation_stack():
                raise Exception("CloudFormation stack setup failed")

            # 2. Setup AWS Batch resources
            if not self._setup_batch_resources():
                raise Exception("AWS Batch resources setup failed")

            # 3. Setup S3 buckets
            if not self._setup_s3_buckets():
                raise Exception("S3 buckets setup failed")

            # 4. Setup IAM roles
            if not self._setup_iam_roles():
                raise Exception("IAM roles setup failed")
    ...
    def _rollback_setup(self) -> None:
        """Rollback infrastructure setup on failure."""
        try:
            print("🔄 Rolling back infrastructure setup...")
